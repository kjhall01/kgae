...
def bootstrap_rel_l2_dist(
    ...
    norm = np.sqrt((full_mean**2).sum(['lat', 'lon']))
    ests, cors = [], []
    for _ in range(n_resamples):
       ...
       #idx = moving_block_bootstrap_indices(rng, full_ensemble_size, block_len=block_length)
        ...
        #diff_mag = np.sqrt(((full_boot_mean - full_mean)**2).sum(['lat', 'lon'])) / norm
        diff_mag = xr.corr(full_boot_mean, full_mean, dim=['lat', 'lon'])
...
if __name__ == "__main__":
    experiment = '../scripts/large-ensemble-2-1940-2014'
    n_ensemble = 100
    n_bootstraps = 1000
    overwrite = False
    cl = 0.05
    ...
    for ci, tendency_mode in enumerate(modes_to_examine):
        ...
        for ri, predictor_mode in enumerate(modes_to_examine, start=1):
            ...
            if ci == 0:
                ax.set_ylabel(r"Pattern Correlation ($\beta$)")
            else:
                ax.set_ylabel("")


    plt.savefig("large-ens-approximation.png", dpi=300)
    plt.show()

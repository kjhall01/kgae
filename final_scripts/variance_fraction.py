...
if __name__ == "__main__":
    ...
    references = open_references('xval', 'large-ensemble-2-1940-2014')
    refs_test = open_references('test', 'large-ensemble-2-1940-2014')
    ...
    for split in splits: 
        ...
        for seed in range(5):
            for fold in range(5): 
                ...
                validation = xr.open_dataset(f'progressive-split/{split}/seed{seed}/fold{fold}/val_reconstructions.nc')['__xarray_dataarray_variable__']
                ...
                if seed ==0: 
                    validation = xr.open_dataset(f'progressive-split/{split}/seed{seed}/fold{fold}/pca_val_reconstructions.nc')['sst']
                    variance_fraction = validation.var('time').sum(['lat', 'lon']) / references.sel(time=validation.time).var('time').sum(['lat', 'lon'])
    ...
    plt.legend()
    plt.savefig('variance_fraction.png')
    plt.show() 

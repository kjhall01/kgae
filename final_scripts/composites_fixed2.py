...
def _common_extent_from_coords(da):
    """Return a common [lon_min, lon_max, lat_min, lat_max] extent from data coords."""
...
def plot_six_panel_composites(
    ...
    common_extent = _common_extent_from_coords(ssta)
    ...
    for r in range(2):
        ...
        for c in range(3):
            ...
            #if common_extent is not None:
            ax.set_extent([-60, 100, -60, 70], crs=ccrs.PlateCarree(central_longitude=180))
            #else:
            #    ax.set_global()
    ...
    def contour_sign(ax, da, levels, **kw):
        ...
        cs_pos = ax.contour(da["lon"], da["lat"], da, levels=pos, 
                            linewidths=0.9, linestyles="-", **kw)
        cs_neg = ax.contour(da["lon"], da["lat"], da, levels=neg, 
                            linewidths=0.9, linestyles="--", **kw)

...
for init in range(N):
    ...
    tc3.append(tc2)
tc2 = xr.concat(tc3, 'initialization').assign_coords({'initialization': np.arange(N)})
...
tc3 = xr.concat([kgae.open_pdo().sel(time=slice("1940-01-01", pd.Timestamp(2014,12,31))).mean('dataset'), kgae.open_oni().sel(time=slice("1940-01-01", pd.Timestamp(2014,12,31))), kgae.open_npi().rolling(time=24, center=False).sum().sel(time=slice("1940-01-01", pd.Timestamp(2014,12,31)))], 'mode')
tc3 = tc3.assign_coords({'mode': ['Decadal', 'Interannual', 'Quasibiennial']}).expand_dims('initialization')
tc3 = tc3 - tc3.mean('time')

tc2 = xr.concat([tc2.isel(recursion=2, drop=True).mean('initialization').sel(mode='Decadal'), tc2.isel(recursion=0, drop=True).mean('initialization').sel(mode='Interannual'), tc2.isel(recursion=0, drop=True).mean('initialization').sel(mode='Quasibiennial')], 'mode')

if var == 'sst':
    sst = xr.open_dataset('~/Desktop/Data/era5/era5.sst.pacific.1x1.1940-2023.nc').sst.sel(time=slice(None, pd.Timestamp(2014,12,31)))
    #sst = sst.sel(time=slice('1958-01-01', '2014-12-31'))
  ...
  #  tc2 = tc2.isel(time=[i for i in range(tc2.time.shape[0]) if i % 12 in [7, 0, 1]])
   ...
   # tc2 = tc2.isel(time=[i for i in range(tc2.time.shape[0]) if i % 12 in [7, 0, 1]])
    ...
    tauy, fit, gwm, p  = kgae.global_detrend(tauy, deg=2)
    tauy , mc = kgae.remove_climo(tauy)
    taux = xr.open_dataset('~/Desktop/Data/oras5/oras5.taux.195801-201412.nc').taux 
    taux, fit, gwm, p  = kgae.global_detrend(taux, deg=2)
    taux , mc = kgae.remove_climo(taux)
    tc2 = tc2.sel(time=slice('1958-01-01', '2014-12-31'))
elif var == 'zonal_wind_stress':
    ssta = xr.open_dataset('~/Desktop/Data/oras5/oras5.taux.195801-201412.nc').taux 
    ssta, fit, gwm, p  = kgae.global_detrend(ssta, deg=2)
    ssta , mc = kgae.remove_climo(ssta)
    tc2 = tc2.sel(time=slice('1958-01-01', '2014-12-31'))
elif var == 'u10':
...
if var == 'wind' or add_wind: 
    ...
    v10a , mc = kgae.remove_climo(v10a) 
    
    wind = xr.Dataset({'u': u10a, 'v': v10a})#.sel(time=slice('1958-01-01', '2014-12-31'))
    wind = wind.isel(lat=slice(None, None, 5), lon=slice(None, None, 5))#.sel(time=slice('1958-01-01', '2014-12-31')) # subsample to 5 degree resolution
   # tc2 = tc2.sel(time=slice('1958-01-01', '2014-12-31'))
    #wind = wind.sel(time=slice('1958-01-01', '2014-12-31'))
...
neg_neg_composite_selection = ( ( tc2.sel(mode=y) < tc2.sel(mode=y).quantile(1-confidence_level, 'time').values ) & 
         #\( tc2\.mean\('initialization'\)\.sel\(mode=z\) > tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(1-confidence_level, 'time'\)\.values \) & \( tc2\.mean\('initialization'\)\.sel\(mode=z\) < tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(confidence_level, 'time'\)\.values \) &
        ( tc2.sel(mode=x) <  tc2.sel(mode=x).quantile(1-confidence_level, 'time').values )  )
...
#neg_neg_composite_selection = neg_neg_composite_selection.roll(time=12) 
#neg_neg_composite_selection[:12] = False
...
neg_neg_composite_selection = ( ( tc2.sel(mode=y) < tc2.sel(mode=y).quantile(1-confidence_level, 'time').values ) & 
         #\( tc2\.mean\('initialization'\)\.sel\(mode=z\) > tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(1-confidence_level, 'time'\)\.values \) & \( tc2\.mean\('initialization'\)\.sel\(mode=z\) < tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(confidence_level, 'time'\)\.values \) &
        ( tc2.sel(mode=x) >  tc2.sel(mode=x).quantile(confidence_level, 'time').values )  )
n = int(neg_neg_composite_selection.sum() .values)
print('samples in ', n , np.sum(neg_neg_composite_selection | ~neg_neg_composite_selection))
#neg_neg_composite_selection = neg_neg_composite_selection.roll(time=12) 
#neg_neg_composite_selection[:12] = False
...
neg_neg_composite_selection = ( ( tc2.sel(mode=y) > tc2.sel(mode=y).quantile( confidence_level, 'time').values ) & 
         #\( tc2\.mean\('initialization'\)\.sel\(mode=z\) > tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(1-confidence_level, 'time'\)\.values \) & \( tc2\.mean\('initialization'\)\.sel\(mode=z\) < tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(confidence_level, 'time'\)\.values \) &
        ( tc2.sel(mode=x) >  tc2.sel(mode=x).quantile(confidence_level, 'time').values )  )
n = int(neg_neg_composite_selection.sum() .values)
print('samples in ', n , np.sum(neg_neg_composite_selection | ~neg_neg_composite_selection))
...
neg_neg_composite_selection = ( ( tc2.sel(mode=y) > tc2.sel(mode=y).quantile(confidence_level, 'time').values ) & 
         #\( tc2\.mean\('initialization'\)\.sel\(mode=z\) > tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(1-confidence_level, 'time'\)\.values \) & \( tc2\.mean\('initialization'\)\.sel\(mode=z\) < tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(confidence_level, 'time'\)\.values \) &
        ( tc2.sel(mode=x) > tc2.sel(mode=x).quantile(1-confidence_level, 'time').values ) & ( tc2.sel(mode=x) < tc2.sel(mode=x).quantile(confidence_level, 'time').values ) )
n = int(neg_neg_composite_selection.sum() .values)
print('samples in ', n , np.sum(neg_neg_composite_selection | ~neg_neg_composite_selection))
#neg_neg_composite_selection = neg_neg_composite_selection.roll(time=12) 
#neg_neg_composite_selection[:12] = False
...
#  middle panel 
neg_neg_composite_selection = ( #( tc2.sel(mode=y) < tc2.sel(mode=y).quantile(confidence_level, 'time').values ) & 
                               #( tc2.sel(mode=y) > tc2.sel(mode=y).quantile(1-confidence_level, 'time').values ) &
         #\( tc2\.mean\('initialization'\)\.sel\(mode=z\) > tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(1-confidence_level, 'time'\)\.values \) & \( tc2\.mean\('initialization'\)\.sel\(mode=z\) < tc2\.mean\('initialization'\)\.sel\(mode=z\)\.quantile\(confidence_level, 'time'\)\.values \) &
        ( tc2.sel(mode=x) > tc2.sel(mode=x).quantile(1-confidence_level, 'time').values ) &
         ( tc2.sel(mode=x) < tc2.sel(mode=x).quantile(confidence_level, 'time').values ) )
n = int(neg_neg_composite_selection.sum() .values)

...
if False:
    tc2 = xr.concat([kgae.open_pdo().sel(time=slice("1940-01-01", pd.Timestamp(2014,12,31))).mean('dataset'), kgae.open_oni().sel(time=slice("1940-01-01", pd.Timestamp(2014,12,31))), kgae.open_npi().rolling(time=24, center=False).sum().sel(time=slice("1940-01-01", pd.Timestamp(2014,12,31)))], 'mode')
...
fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(6,6), subplot_kw={'projection': ccrs.PlateCarree(central_longitude=180)})
letters0 = ['a)', 'b)']
letters1 = ['c)', 'd)']
titles = [ 'Interannual', 'Decadal']
x = 'Decadal'
y = 'Interannual'

neg_neg_composite_selection = ( ( tc2.mean('initialization').sel(mode=y) < tc2.mean('initialization').sel(mode=y).quantile(1-confidence_level, 'time').values ) & 
        #( np.abs(tc2.mean('initialization').sel(mode='Quasibiennial')) <  tc2.mean('initialization').sel(mode='Quasibiennial').std('time') ) &
        ( tc2.mean('initialization').sel(mode=x) <  tc2.mean('initialization').sel(mode=x).quantile(1-confidence_level, 'time').values )  )
...
if var != 'wind':
    ...
    p = neg_neg_composite.plot(ax=ax[1,0], cmap='RdBu_r', vmin=-vmax, vmax=vmax, add_colorbar=False)
...
neg_neg_composite_selection = ( ( tc2.mean('initialization').sel(mode=y) < tc2.mean('initialization').sel(mode=y).quantile(1-confidence_level, 'time').values ) & 
        #( np.abs(tc2.mean('initialization').sel(mode='Quasibiennial')) <  tc2.mean('initialization').sel(mode='Quasibiennial').std('time') ) &
        ( tc2.mean('initialization').sel(mode=x) >  tc2.mean('initialization').sel(mode=x).quantile(confidence_level, 'time').values )  )
n = int(neg_neg_composite_selection.sum() .values)
print('samples in ', n , np.sum(neg_neg_composite_selection | ~neg_neg_composite_selection))
neg_neg_composite = ssta.isel(time=neg_neg_composite_selection).mean('time') 
...
if var != 'wind':
    ...
    p = neg_neg_composite.plot(ax=ax[1,1], cmap='RdBu_r', vmin=-vmax, vmax=vmax, add_colorbar=False)
...
neg_neg_composite_selection = ( ( tc2.mean('initialization').sel(mode=y) > tc2.mean('initialization').sel(mode=y).quantile( confidence_level, 'time').values ) & 
        #( np.abs(tc2.mean('initialization').sel(mode='Quasibiennial')) <  tc2.mean('initialization').sel(mode='Quasibiennial').std('time') ) &
        ( tc2.mean('initialization').sel(mode=x) >  tc2.mean('initialization').sel(mode=x).quantile(confidence_level, 'time').values )  )
n = int(neg_neg_composite_selection.sum() .values)
print('samples in ', n , np.sum(neg_neg_composite_selection | ~neg_neg_composite_selection))
...
if var != 'wind':
    ...
    p = neg_neg_composite.plot(ax=ax[0,1], cmap='RdBu_r', vmin=-vmax, vmax=vmax, add_colorbar=False)
...
ax[0,0].set_title(letters0[0], loc='left', fontweight='bold')
ax[0,1].set_title(letters0[1], loc='left', fontweight='bold')   
ax[1,0].set_title(letters1[0], loc='left', fontweight='bold')
ax[1,1].set_title(letters1[1], loc='left', fontweight='bold')

ax[0,0].text(-0.03, 0.5, 'Positive ' + y, va='center', ha='right', rotation=90, transform=ax[0,0].transAxes)
ax[1,0].text(-0.03, 0.5, 'Negative ' + y, va='center', ha='right', rotation=90, transform=ax[1,0].transAxes)
ax[1,1].text(0.5, -0.1, 'Positive ' + x, va='center', ha='center', transform=ax[1,1].transAxes)
ax[1,0].text(0.5, -0.1, 'Negative ' + x, va='center', ha='center', transform=ax[1,0].transAxes)

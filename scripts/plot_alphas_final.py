...
# ----------------------------
comp_sig_thresh = 0.99
comp_sig_thresh_hatch = 1.1
...
def _bootstrap_sigmask_linear_comp_one_seed(
    ...
    Seed-level significance for linear/symmetric composite:
      comp = E[X|pos, neutral] - E[X|neg, neutral]
    using bootstrap over time indices. Returns sigmask(lat, lon) where CI excludes 0.
    """
    ...
    sigmask = (lo > 0) | (hi < 0)
    return xr.DataArray(sigmask, coords={"lat": X["lat"], "lon": X["lon"]}, dims=("lat", "lon"))

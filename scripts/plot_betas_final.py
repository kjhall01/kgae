...
def _pure_even_sigmask_one_seed(
    ...
    where "others neutral" means: for all j != i, |z_j| < neutral_thresh * std_j.

    Returns:
        comp_point(lat, lon), sigmask95(lat, lon)  (CI excludes 0)
    """
    ...
    # three disjoint-ish regimes for the target mode, all requiring others_neutral
    ...
    # ---- bootstrap for sigmask ----
    tN = X.sizes["time"]
    X_np = X.values
    pos_np = pos_mask.values.astype(bool)
    neg_np = neg_mask.values.astype(bool)
    neu_np = neu_mask.values.astype(bool)
    ...
    for b in range(n_boot):
        ...
        pos_m = (
            np.nanmean(X_b[pos_b, :, :], axis=0)
            if pos_b.any()
            else np.full((X.sizes["lat"], X.sizes["lon"]), np.nan, np.float32)
        )
        neg_m = (
            np.nanmean(X_b[neg_b, :, :], axis=0)
            if neg_b.any()
            else np.full((X.sizes["lat"], X.sizes["lon"]), np.nan, np.float32)
        )
        neu_m = (
            np.nanmean(X_b[neu_b, :, :], axis=0)
            if neu_b.any()
            else np.full((X.sizes["lat"], X.sizes["lon"]), np.nan, np.float32)
        )
    ...
    sigmask = (lo > 0) | (hi < 0)
    sigmask = xr.DataArray(sigmask, coords={"lat": X["lat"], "lon": X["lon"]}, dims=("lat", "lon"))

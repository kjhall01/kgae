...
if __name__ == "__main__":
    ...
    latents.sel(mode=3).mean('seed').plot.line(ax=ax[0], x='time', color='r', linestyle='--', add_legend=False, alpha=0.7)
    latents.sel(mode=4).mean('seed').plot.line(ax=ax[0], x='time', color='b', linestyle='--', add_legend=False, alpha=0.7)
    latents.sel(mode=5).mean('seed').plot.line(ax=ax[1], x='time', color='g', linestyle='--', add_legend=False, alpha=0.7)

    pdo = kgae.open_pdo().mean('dataset')
    pdo, _ = xr.align(pdo, oni)

    oni.plot.line(ax=ax[0], x='time', linestyle='-', color='k')
    pdo.plot.line(ax=ax[1], x='time', linestyle='-', color='k')
    ...
    handles = [
        # Model colors
        Line2D([0], [0], color="g", linewidth=3, linestyle='-', label='DE'),
        Line2D([0], [0], color='r', linewidth=3, linestyle='--', label="QB"),
        Line2D([0], [0], color='b',  linewidth=3, linestyle='--', label="IA"),
        Line2D([0], [0], color="k", linewidth=3, linestyle='-', label='ONI'),
        Line2D([0], [0], color="k", linewidth=3, linestyle='-', label='PDO'),
    ]
    fig.legend(
        handles=handles,
        loc="lower center",
    ...
    plt.savefig('enso-qb-ia-ts.pdf')
    plt.tight_layout()
    plt.show()
    import sys; sys.exit() 

    fig, axes, out = make_figure(latents, oni, mode_qb=3, mode_ia=4, nlags=36)
    fig.savefig("qb_ia_oni_seasonal_amp_and_lag.pdf", bbox_inches="tight")
    plt.show()

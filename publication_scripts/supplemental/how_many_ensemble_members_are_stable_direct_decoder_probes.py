...
"""
Estimate how many ensemble members are needed for split-half stability of
explicit decoder-probe patterns.
...
def bootstrap_rel_l2_dist(
    ...
    del block_length  # retained for API compatibility with original script
    ...
    for boot_idx in range(n_resamples):
        ...
        idx1 = idx[ensemble_size:]

        boot_mean1 = da_full.isel(**{dim: idx1}).mean(dim)
        boot_mean2 = da_full.isel(**{dim: idx2}).mean(dim)
        corr = xr.corr(boot_mean1, boot_mean2, dim=["lat", "lon"])
        ests.append(corr)
...
def _get_feature_template(experiment_dir: Path) -> xr.DataArray:
    """Recover the original stacked feature order used during training / decoding."""
...
def decode_model(model, z_np: np.ndarray, device: str = "cpu") -> np.ndarray:
    """
    Decode a reference-oriented latent vector z_np of shape (latent_dim,).
    Saved alignment signs are applied internally.
    """
...
def nonlinear_response_target_given_bg(
    ...
    *,
    ...
    with all other latents set to zero.
    """
...
def state_dependence_probe(
    ...
    *,
    ...
    """
    S_{i<-j} = [N_i(+a_j) - N_i(-a_j)] / (2 a_j)

    This reproduces the self-state-dependence quantity from figure3.py when
    i == j, and the cross-mode state-dependence quantity from figure5.py when
    i != j.
    """
...
def _row_ylabel(background_mode: int | None) -> str:
    ...
    return f"Pattern Correlation\n({MODE_LABELS[int(background_mode)].replace(' Mode', '')} impact)"
...
if __name__ == "__main__":
    ...
    plt.savefig("split-half-experiment-direct-decoder-probes.pdf")
    plt.show()
